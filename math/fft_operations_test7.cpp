...
vector<int> chirpTransform(poly& p, int g, int k) {
	int inv_g=inv(g), n=((int)(p).size())-1, sz=min(k,MOD-1);
	poly ap(n+1), bp(n+sz);
...
int main(){FIN;
	...
	poly c(n);
	for (int i = 0, _n = n; i < _n; ++i) cin>>c[i];

	int x=1;
	for (int i = 0, _n = n; i < _n; ++i) c[i]=mulmod(c[i],x), x=mulmod(x,a);
	
	vector<int> ans;

	if(r==0){
		ans=vector<int>(k,c[0]);
		for (int i = 1, _n = n; i < _n; ++i) ans[0]=addmod(ans[0], c[i]);
	}
	else{
